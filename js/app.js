...
generateBtn.addEventListener("click",function(event){
  ...
  let randomtextindex=Math.floor(Math.random()*randomText.length)
  if(userInputValue>9||isNaN(userInputValue)===true||userInputValue<0||userInputValue===""){
  result.innerHTML=randomText[randomtextindex]
  }
  else{
    let slicedResult=randomText.slice(0,userInputValue)
    let finalResult=slicedResult.map(function(index){
    return "<p>"+index+"</p>"
    }).join("")
